...
parser.add_argument('--name', default='Ablation_OneWay_Dino2CNN', type=str, help='保存文件夹的名字')
        ...
        for data in dataloaders:
            ...
            with autocast():
                ...
                for i in range(opt.block):
                    ...
                    # 2. 单向互学习 Loss (KL Divergence)
                    # 关键点：out_dino 必须加上 .detach()，使其成为一个纯粹的“软标签”生成器
                    # 这样梯度只会回传给 CNN，DINO 的权重不会因为教 CNN 而被改变
                    loss_kl_cnn_learn = F.kl_div(
                        F.log_softmax(out_cnn, dim=1),  # 学生 (CNN) 用 log_softmax
                        F.softmax(out_dino.detach(), dim=1),  # 老师 (DINO) 截断梯度
                        reduction='batchmean'
                    )

                    # 🔥 终极 Loss 融合：(双分支 ID + 单向 KL)
                    loss += loss_id_cnn + loss_id_dino + 1.0 * loss_kl_cnn_learn
